  ...
  return (
    <>
      <style>{`
        ...
        .slide-in-right { animation: slideInRight 0.3s cubic-bezier(0.22,1,0.36,1) forwards; }
      ...
      {/* Announcement Bar */}
      <div className="bg-terracotta-deep text-creamy-ivory py-2 px-4 text-center text-xs font-bold tracking-[0.2em] uppercase border-b border-burnished-copper/30 w-full">
        <EditableText 
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 bg-earthy-brown/90 backdrop-blur-xl border-b border-burnished-copper/20 px-6 md:px-16 py-6 w-full">
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
          <a href="/" className="flex flex-col items-center select-none cursor-pointer">
            <span className="material-symbols-outlined text-bronze-metallic text-4xl mb-1">temple_hindu</span>
          ...
          <nav className="hidden lg:flex items-center gap-12">
            <a className="font-decorative text-sm text-bronze-metallic hover:text-creamy-ivory transition-all tracking-widest" href="/new-arrivals">NEW ARRIVALS</a>
            <a className="font-decorative text-sm text-bronze-metallic hover:text-creamy-ivory transition-all tracking-widest" href="/sarees">SILK SAREES</a>
            <a className="font-decorative text-sm text-bronze-metallic hover:text-creamy-ivory transition-all tracking-widest" href="/concierge">CONCIERGE</a>
            <a className="font-decorative text-sm text-bronze-metallic hover:text-creamy-ivory transition-all tracking-widest" href="/about">ABOUT</a>
          </nav>
          <div className="flex items-center gap-8">
            <button onClick={() => setIsSearchOpen(true)} className="text-bronze-metallic hover:scale-110 transition-transform" aria-label="Search">
              <span className="material-symbols-outlined text-3xl">search</span>
            </button>
            <button onClick={() => setIsProfileOpen(true)} className="text-bronze-metallic hover:scale-110 transition-transform relative" aria-label="Profile">
            ...
            <a href="/cart" className="text-bronze-metallic hover:scale-110 transition-transform relative block" aria-label="Cart">
      ...
      {/* ── SEARCH OVERLAY ─────────────────────────────────────── */}
      {isSearchOpen && (
        <div className="fixed inset-0 bg-black/70 flex flex-col items-center pt-24 z-50 p-4 backdrop-blur-md" onClick={closeSearch}>
          <div className="relative w-full max-w-2xl fade-in-down" onClick={e => e.stopPropagation()}>
            {/* Search bar */}
                    ...
                    <div className="space-y-2">
                      {POPULAR_SEARCHES.map(s => (
                        <button key={s} onClick={() => setSearchQuery(s)}
                          className="flex items-center gap-3 text-sm font-serif italic text-creamy-ivory/60 hover:text-creamy-ivory transition-colors w-full text-left">
                          <span className="material-symbols-outlined text-sm text-bronze-metallic/40">trending_up</span>
                          {s}
                        </button>
                      ))}
                    </div>
      ...
      {isProfileOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-end" onClick={() => setIsProfileOpen(false)}>
          <div className="w-full max-w-sm bg-earthy-brown border-l border-burnished-copper/30 shadow-2xl flex flex-col slide-in-right" onClick={e => e.stopPropagation()}>
            {/* Header */}
              ...
              {user ? (
                <div className="space-y-6">
                  <div className="flex items-center gap-4 p-4 bg-terracotta-deep/10 border border-burnished-copper/20">
                    <div className="w-12 h-12 rounded-full bg-terracotta-deep flex items-center justify-center text-creamy-ivory font-decorative text-xl">
                      {user.name[0].toUpperCase()}
                    </div>
                    <div>
                      <p className="font-decorative text-creamy-ivory text-sm">{user.name}</p>
                      <p className="font-serif italic text-xs text-bronze-metallic/60">{user.email}</p>
                    </div>
                  </div>
                  <nav className="space-y-2">
                    {[
                      ...
                      { icon: "loyalty",      label: "My Rewards",href: "/concierge" },
                    ].map(item => (
