...
import { FadeIn, StaggerContainer, ScaleIn } from '@/components/AnimationWrapper';

export default function HomeFallback({ content, outstaticProducts }: { content?: SiteContent, outstaticProducts?: any[] }) {
  ...
  const mergedProducts = (outstaticProducts && outstaticProducts.length > 0) ? outstaticProducts : (displayContent ? getMergedProducts(displayContent) : products);
      ...
      <div className="bg-earthy-brown text-creamy-ivory font-sans min-h-screen">
        ...
        <section className="relative h-[921px] w-full overflow-hidden flex items-center">
          ...
          <div className="relative max-w-7xl mx-auto px-8 md:px-20 z-10">
            <FadeIn direction="left" className="border-l-4 border-bronze-metallic pl-8 py-4">
              ...
              <EditableText 
                ...
                className="text-6xl md:text-9xl font-decorative leading-none mb-8 gold-gradient-text drop-shadow-2xl block" 
              />
              <EditableText 
                tagName="p" 
                ...
                className="text-xl md:text-2xl font-serif italic max-w-2xl mb-12 text-creamy-ivory leading-relaxed block" 
              />
              <a href="/sarees" className="group relative px-12 py-5 overflow-hidden inline-block">
        ...
        <section className="py-32 bg-terracotta-deep/10 border-y border-burnished-copper/10">
          <div className="max-w-screen-2xl mx-auto px-8">
            ...
            <div className="relative group/carousel">
              ...
              <StaggerContainer ref={signatureRef} className="flex justify-center flex-wrap gap-6 px-4">
              {(displayContent?.festivals && displayContent.festivals.length > 0 ? displayContent.festivals : [
                ...
                <FadeIn key={index} direction="up" delay={index * 0.1} className="group text-center card-lift block p-4 flex-shrink-0 w-44 md:w-52">
                  <a href={`/sarees?festival=${item.key}`}>
        ...
        <section className="py-32 bg-earthy-brown relative">
          <div className="max-w-7xl mx-auto px-8">
            ...
            <div className="relative group/carousel">
              ...
              <StaggerContainer ref={bestsellersRef} className="flex overflow-x-auto snap-x snap-mandatory flex-nowrap no-scrollbar gap-8 pb-4 px-4 -mx-4 scroll-smooth">
              {mergedProducts.slice(0, 4).map((item, index) => (
                <FadeIn key={index} direction="up" delay={index * 0.15} className="group relative bg-terracotta-deep/20 p-4 border border-burnished-copper/10 hover:border-burnished-copper/50 transition-all ruby-shadow card-lift flex-shrink-0 w-64 md:w-72 snap-center">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-2xl font-decorative text-bronze-metallic">
                      ₹<EditableText 
                        ...
                        onChange={(v: string) => {
                          updateField(['products', index.toString(), 'id'], item.id.toString());
                          updateField(['products', index.toString(), 'price'], v);
                        }} 
                        />
                    </span>
                  </div>
        ...
        <section className="bg-earthy-brown/40 py-20 border-y border-burnished-copper/30">
          <StaggerContainer className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-4 gap-12 text-center">
            {[
              ...
              <FadeIn key={index} direction="up" delay={index * 0.1} className="group cursor-default">
          ...
          <div className="max-w-7xl mx-auto px-8">
            ...
            <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <ScaleIn key={index} delay={index * 0.15} onClick={() => setActiveVideo(video.id)} className="relative group aspect-square ornate-border overflow-hidden cursor-pointer">
